...
function getImpactAnalysis(transaction, inputoutput, scenario) {
   ...
   *
   * Args:
   *      transaction =>  type: array of array| transaction matrix
   *      inputoutput => type: array of array | ouput vector
   *      scenario => type: array of array | scenario vector
   *
   * return => type: Matrix mathjs obj
   */
  scenario = mathjs.transpose(scenario) // transpose matrix
...
=======
  ...
  const result = []
>>>>>>> bee24d3dd36c29479edcc40fe54e454604774f76

  for (let i = 0; i < inputoutput.length; i++) {
    result.push({
      sectorforward: forwardLinkage[i][0],
      valueforward: forwardLinkage[i][1],
      sectorbackward: backwardLinkage[i][0],
      valuebackward: backwardLinkage[i][1],
    });
  }
<<<<<<< HEAD
  return result;
